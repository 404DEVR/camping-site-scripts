...
def train_model(X_train, y_train):
    """Train Artificial Neural Network classifier with feature scaling."""
    print(f"\nTraining Artificial Neural Network classifier...")
    ...
    # Encode labels for ANN
    label_encoder = LabelEncoder()
    y_train_encoded = label_encoder.fit_transform(y_train)
    
    # Calculate hidden layer size based on input features
    input_size = X_train.shape[1]
    hidden_size = min(100, max(10, input_size // 2))  # Adaptive hidden layer size
    
    model = MLPClassifier(
        hidden_layer_sizes=(hidden_size, hidden_size//2),  # Two hidden layers
        activation='relu',
        solver='adam',
    ...
    print(f"Network architecture: {input_size} -> {hidden_size} -> {hidden_size//2} -> {len(y_train.unique())}")
    
    model.fit(X_train_scaled, y_train_encoded)
    print("Training completed!")
    print(f"Training converged in {model.n_iter_} iterations")
    
    return model, scaler, label_encoder

def evaluate_model(model, scaler, label_encoder, X_train, X_test, y_train, y_test):
...
def main():
    ...
    train_file = "../data_splits/train_data.csv"  # Your training CSV file
    test_file = "../data_splits/test_data.csv"   # Your test CSV file
    ...
    print("CUSTOM TRAIN/TEST ANN PIPELINE")
    print("=" * 40)
    ...
    try:
        ...
        # Train model
        model, scaler, label_encoder = train_model(X_train, y_train)
        
        # Evaluate model
        accuracy, predictions = evaluate_model(model, scaler, label_encoder, X_train, X_test, y_train, y_test)
...
if __name__ == "__main__":
    print("Starting ANN ML Pipeline...")
    print("Checking dependencies...")
    
    # Check for required dependencies (openpyxl not needed for CSV files)
    print("All dependencies available for CSV processing")
